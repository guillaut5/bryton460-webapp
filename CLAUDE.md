...
- Fichier de travail v0.10 : `src/` (modules ES) + `index.html` → `npm run build` → `dist/index.html`
- Archive proto : `proto_html/bryton.html` (v0.2), `proto_html/bryton_v0.1.html` (ne pas modifier)
- Tags git : `v0.1`, `v0.2`, `v0.4`, `v0.5`, `v0.6`, `v0.7`, `v0.8`, `v0.9`, `v0.10`
...
- v0.10 = aperçu remplacé par une vraie carte Leaflet + tuiles OSM (zoom/pan). Ajoute `leaflet`
  comme 2e dépendance npm (après `jszip`) et rend la fonctionnalité "aperçu" dépendante du
  réseau pour son rendu complet (dégradation propre : tracé + marqueurs restent visibles hors
  ligne, cf. `src/ui/routePreview.js`)
